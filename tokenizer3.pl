...
#  print $file . "\n";
  foreach my $comparison(@files) {
    if($file ne $comparison) {
      ...
      my $s1 = returntoken($file);

      my $s2 = returntoken($comparison);


      print "\n****";
      my @result = lcss($s1, $s2);
...
sub returntoken {
	...
	if($token->[0] eq 'S') {
	  ...
	  elsif($token->[1] eq 'noscript') {
	    $string .= 'n';
	  }
	  elsif($token->[1] eq 'div') {
	    $string .= 'd';
	  }
	  ...
	  elsif($token->[1] eq 'select') {
	    $string .= 'l';
	  }
	  elsif($token->[1] eq 'option') {
	    $string .= 'o';
	  }
